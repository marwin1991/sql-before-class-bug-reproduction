...
@SpringBootTest
@Sql(value = "classpath:test.sql", executionPhase = Sql.ExecutionPhase.BEFORE_TEST_CLASS)
public class WithSqlAnnotationTest {
    ...
    void varShouldBeEqualTo1(){
        // validate if sql was executed
        List<String> result = new NamedParameterJdbcTemplate(dataSource).queryForList("SELECT name FROM test_tabel", new HashMap<>(), String.class);
